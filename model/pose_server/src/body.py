...
class Body(object):
    ...
    def __call__(self, oriImg):
        multiplier = [x * config.boxsize / oriImg.shape[0] for x in config.scale_search]
        ...
        for m in range(len(multiplier)):
            ...
            imageToTest_padded, pad = util.padRightDownCorner(imageToTest, config.stride, config.padValue)
            ...
            heatmap = cv2.resize(heatmap, (0, 0), fx=config.stride, fy=config.stride, interpolation=cv2.INTER_CUBIC)
            heatmap = heatmap[:imageToTest_padded.shape[0] - pad[2], :imageToTest_padded.shape[1] - pad[3], :]
            heatmap = cv2.resize(heatmap, (oriImg.shape[1], oriImg.shape[0]), interpolation=cv2.INTER_CUBIC)

            # paf = np.transpose(np.squeeze(net.blobs[output_blobs.keys()[0]].data), (1, 2, 0))  # output 0 is PAFs
            paf = np.transpose(np.squeeze(Mconv7_stage6_L1), (1, 2, 0))  # output 0 is PAFs
            paf = cv2.resize(paf, (0, 0), fx=config.stride, fy=config.stride, interpolation=cv2.INTER_CUBIC)
        ...
        for part in range(18):
            ...
            peaks_binary = np.logical_and.reduce(
                (one_heatmap >= map_left, one_heatmap >= map_right, one_heatmap >= map_up, one_heatmap >= map_down, one_heatmap > config.thre1))
            ...
            if (nA != 0 and nB != 0):
                ...
                for i in range(nA):
                    for j in range(nB):
                        ...
                        criterion1 = len(np.nonzero(score_midpts > config.thre2)[0]) > 0.8 * len(score_midpts)
        ...
        # Select only one subset which have the largest area of the bounding box
    ...
    def compare(self,oriImg,compareImg,guide=True,gpt=True):
        ...
        vectors1 = self.getVectors(keypoints1,config.limbSeqToCompare)
        
        candidate2, subset2 = self.__call__(compareImg)
    ...
    def generateKeypoints(self,img):
        candidate, subset = self.__call__(img)
        keypoints = self.getKeypointList(candidate,subset)
        return keypoints
    ...
    def selectOneKeypointSets(self,candidate,subset):
        for i in range(len(subset)):
            temp = candidate[subset[i][subset[i]>-1][:-2].astype(int)]
            subset[i][-2] = temp[:,0].max() - temp[:,0].min()
            subset[i][-1] = temp[:,1].max() - temp[:,1].min()
        subset = subset[np.argmax(subset[:,-1]*subset[:,-2])]
        return subset[:-2].astype(int)

    def getKeypointList(self,candidate,subset):
